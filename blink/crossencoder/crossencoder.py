...
class CrossEncoderRanker(torch.nn.Module):
    ...
    def forward(self, input_idx, label_input, context_len,inference_only=False, label_is_NIL_input=None, use_original_classification=True, use_NIL_classification=False, lambda_NIL=0.5, use_score_features=True,use_score_pooling=False,use_men_only_score_ft=False,use_extra_features=False,mention_matchable_fts=None): # not calculating loss if inference_only
        scores = self.score_candidate(input_idx, context_len)
        
        if use_NIL_classification:
            if use_score_pooling:
                scores_w_stats = self.min_max_ave_pooling(scores)
                #print('pooled')
            if use_men_only_score_ft:
                men_score = self.get_score_from_mention_emb(input_idx, context_len)    
                #print('men_score:',men_score)            
            else:
                men_score = None
            scores_NIL = self.score_NILs(
                scores_w_stats if use_score_pooling else scores, # use scores with pooled stats for NIL classification if chosen to
                use_score_features=use_score_features,
                use_extra_features=use_extra_features,
                use_men_only_score_ft=use_men_only_score_ft,
                mention_matchable_fts=mention_matchable_fts,
                men_score=men_score)
            #print('scores_NIL:',scores_NIL)
        #print('label_input:',label_input)
        #print('label_is_NIL_input:',label_is_NIL_input)
        ...
        if not inference_only:
            assert use_original_classification or use_NIL_classification # at least one part of the loss is turned on

            if use_original_classification:                
                ...
                loss = 0.
                scores.detach() # Tensor.detach(): It returns a new tensor without requires_grad = True. The gradient with respect to this tensor will no longer be computed.

            if use_NIL_classification:
                loss_NIL = F.cross_entropy(scores_NIL,label_is_NIL_input, reduction="mean")            
                #print('loss_NIL:',loss_NIL)            

                loss = loss + lambda_NIL * loss_NIL
                #print('loss_all:',loss)

                return loss, scores, scores_NIL
            else:
                return loss, scores, None
            # to do - add a NIL-based regulariser here later 
        else:
            if use_NIL_classification:
                return scores, scores_NIL
            else:
                return scores, None    
