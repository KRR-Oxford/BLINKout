...
def _save_predictions(args, samples, predictions, scores, local_id2wikipedia_id, title2id, logger=None):
    ...
    for ind_sample, sample in enumerate(samples):
        ...
        if 'CUI-less' in topk_concepts:
            NIL_ent_id_in_cross = topk_concepts.index('CUI-less')
        ...
        mention_row_id_dict['NIL'] = (gold_concept == 'CUI-less') # True or False for the mention
        mention_row_id_dict[row_ind_men] = mention_info_dict
        mention_cand_json_str = json.dumps(mention_row_id_dict)
...
def run(
    ...
    while not stopping_condition:
        ...
        local_id_NIL = wikipedia_id2local_id['CUI-less'] # we added a general out-of-KB / NIL entity to the list - so that all out-of-KB entities share a common ID.
        if args.with_NIL_infer:                        
            #print('args:',args)
        ...
        if args.interactive:
            ...
            if not keep_all:
                ...
                recall_in_KB_at = -1
                if 'CUI-less' in wikipedia_id2ori_local_id:
                    # here NIL is in the entity list
                    x_in_KB = []
                    num_in_KB_tp_slash_all_in_KB = []
                    y_in_KB = []
                    label_id_NIL = wikipedia_id2ori_local_id['CUI-less']
