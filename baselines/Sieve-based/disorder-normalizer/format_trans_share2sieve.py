...
Output format:
filename: 00176-102920-ECHO_REPORT.concept
    ...
    for ind_doc_fn, filename in enumerate(tqdm(list_doc_fns[:num_files_debugging])):
        ...
        for mention_record in mention_records:
            ...
            if doc_fn.endswith('.txt'):
                doc_fn = doc_fn[:len(doc_fn)-len(".txt")]
                doc_concept_fn = doc_fn + '.concept'
            ...
            if not keep_all_positions:
                ...
                positions_new = position_left + '|' + position_right
            else:
                positions_new = '|'.join(positions)    
            mention = ''
                ...
                #    print('\t', 'ctx_right:',len(context_left.split()), context_right)
                    ...
                    if data_split_mark == 'train':
                        ...
                        if mention in dict_mentions_NIL_train_full:
                            continue
            assert concept != ''
            men_str = '||'.join([doc_fn,positions_new,concept_type,mention,concept])
