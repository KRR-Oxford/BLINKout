# this is the code adapted from https://github.com/KRR-Oxford/DeepOnto/blob/main/src/deeponto/onto/prune/pruning.py, it add some further outputs, e.g. concepts not contained in the ontology.
# to run it - go to the DeepOnto repository, replace the original pruning.py with this file.
# example running script below
'''
...
class OntoPruner:
    def __init__(
        ...
        print(self.overlap_count(self.onto,self.preserved_iris))
        list_non_contained_iris = self.get_non_contained_iris(self.onto,self.preserved_iris)
        self.output_to_file(preserved_iris_path + '.non_contained.txt','\n'.join(list_non_contained_iris))
        #sys.exit(0)
    ...
    def simple_prune(self):
        ...
        for cl in tqdm(self.onto.owl.classes()): #342636it [10:22, 550.10it/s]
            #print(cl)
            #print(cl.iri)
    ...
    @staticmethod
    def get_non_contained_iris(onto: Ontology, preserved_iris: List[str]):
        """Get the classess not contained in the ontology
        """
        class_iris = [cl.iri for cl in onto.owl.classes()]
        #preserved_iris_not_contained = [iri for iri in preserved_iris if not iri in set(class_iris) ]
        preserved_iris_not_contained = list(set(preserved_iris).difference(set(class_iris)))
        print('preserved_iris_not_contained:',len(preserved_iris_not_contained))
        return preserved_iris_not_contained

    @staticmethod
    #def isolate_class(self, cl: ThingClass, keep_hierarchy: bool = True):
    def isolate_class(cl: ThingClass, keep_hierarchy: bool = True):
        ...
        if keep_hierarchy:
            ...
            for ch in children:
                ch.is_a += parents
                print(ch.is_a)
                # an example of cl not in child's parents
                # Link the parents and children of the isolated class: http://snomed.info/id/246061005
                # [id.246061005, id.900000000000441003]
                # [id.246061005, id.900000000000441003]
                # [owl.ObjectProperty, id.900000000000441003]

                # interesting example: Link the parents and children of the isolated class: http://snomed.info/id/14914100011910
                # [id.149141000119102, id.225826001 & id.406137001 & id.609096000.some(id.246454002.some(id.41847000)), owl.Thing]
                if cl in ch.is_a: #TODO: if not cl.iri in self.preserved_iris:
                    #if not cl.iri in self.preserved_iris:
                    print('removing',cl)
