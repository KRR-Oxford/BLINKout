...
'''
Input format (two files below):
File 1 - corpus_pubtator_pmids_trng.txt
27773526
27583127
...
File 2 - corpus_pubtator.txt
...
27773526|a|Fibroblast growth factor-1 (FGF-1), a potent human mitogen and insulin sensitizer...
...
def countNILs_for_blink(list_data_json_str):
    num_NILs = 0
...
# count the number of NILs in the list of doc_men_str
def countNILs(list_doc_men_str):
    num_NILs = 0
    for doc_men_str_ in list_doc_men_str:
        data_eles = doc_men_str_.split("||")
        if data_eles[-1] == 'CUI-less':
            num_NILs += 1
    return num_NILs
...
output_data_folder_path = '../data/MedMentions-preprocessed/sieve-%s-%s%s%s' % (data_setting,onto_ver.replace('/','-'), '_filt' if new_NIL_mention_only_for_eval else '', '_syn_full' if add_synonyms_as_ents else '') # for Sieve
    ...
    for doc in tqdm(corpus):
        ...
        if pmid_doc in pmids_list:
            ...
            for entity in entities:
                ...
                positions_formatted = str(mention_pos_start) + '|' + str(mention_pos_end)
                    ...
                    #    print('\t', 'ctx_right:',len(context_left.split()), context_right)
                        ...
                        if data_split_mark == 'trng':
                            ...
                            if mention in dict_mentions_NIL_train_dev:
                                ...
                                continue
                assert concept != ''
                men_str = '||'.join([pmid_doc,positions_formatted,STYs,mention,concept])
                assert len(men_str.split('||')) == 5  
                list_doc_men_str.append(men_str)

            doc_fn = pmid_doc + '.txt'
            doc_concept_fn = pmid_doc + '.concept'
            output_to_file('%s/%s' % (output_data_folder_path_w_split, doc_fn),doc_text)
            num_mention_doc = len(list_doc_men_str)
            num_mention_NIL_doc = countNILs(list_doc_men_str)
            num_mention = num_mention + num_mention_doc # update the whole number of mentions for the data split
            num_mention_NIL = num_mention_NIL + num_mention_NIL_doc # update the whole number of mentions for the data split
            if num_mention_doc > 0: # only output if there is mention
